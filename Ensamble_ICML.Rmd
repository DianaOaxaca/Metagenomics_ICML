...
Ahora creemos una liga simbolica al lugar a donde viven los datos.
...
Una primer cosa que nos gustaria hacer seria ver la calidad de las secuencias a utilizar, para eso vamos a analizar la calidad de las lecturad con fastqc.
...
```{bash eval=F}
fastqc -t 4 $HOME/00.RawData/*.fastq 
```
...
```{bash eval=F}
scp -P Numero_de_puerto usuario@IP:/rurta/a/mi/archivo .
```
...
```{bash eval=F}
java -jar /opt/Trimmomatic-0.39/trimmomatic-0.39.jar PE -threads 4 -phred33 -trimlog $HOME/02.Trimmomatic/triminfo.txt $HOME/00.RawData/SRR10997046_1.fastq $HOME/00.RawData/SRR10997046_2.fastq  $HOME/02.Trimmomatic/SRR10997046_trimm.fastq  $HOME/02.Trimmomatic/SRR10997046_R1_unpair.fastq $HOME/02.Trimmomatic/SRR10997046_R2_trimm.fastq SRR10997046_R2_unpair.fastq ILLUMINACLIP:/opt/Trimmomatic-0.39/adapters/TruSeq2-PE.fa:2:30:10:8:True LEADING:5 TRAILING:5 SLIDINGWINDOW:5:15 MINLEN:50
```
